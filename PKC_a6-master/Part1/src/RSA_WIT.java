		...
		try {
			while ((w = in.read(buffer)) != 0) {
				if (w == NB) {
					 ...
					 * 
					 * System.out.println("\t------------------------------------");
					 * System.out.println("\t (" + message.bitLength() + " bits) message   = " +
					 * message); System.out.println("\t (" + encrypt.bitLength() +
					 * " bits) encrpyted = " + encrypt); System.out.println("\t (" +
					 * decrypt.bitLength() + " bits) decrypted = " + decrypt); System.out.println();
					 */
					...
					// In case we did not encrypt everything, because the last block was not NB
					// size.
					/*
		...
		try {
			...
			while ((ret = in.read(tr)) == ss) { // go in a loop and read (ss) number of bytes from the encrypted file.
				...
				if (data.length > NB) { // dump only the last NB bytes
					...
					for (int r = 0; r < NB; r++)
						gg[r] = (byte) 0; // initialize array to all 0s
