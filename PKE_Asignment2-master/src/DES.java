...
public class DES {
	...
	byte[] iv = new byte[] { (byte) 0x8E, (byte) 0x12, (byte) 0x39, (byte) 0x9C, (byte) 0x07, (byte) 0x72, (byte) 0x6F,
			(byte) 0x5A };

	DES(String passPhrase, String ALG) {
	...
	/*
	 * Create a key like: SecretKey key =
	 * KeyGenerator.getInstance("DES").generateKey(); and pass the key to this
	 * constructor.
	 */
	/*
	 * DES(SecretKey key) {
	 * 
	 * AlgorithmParameterSpec paramSpec = new IvParameterSpec(iv); try { ecipher =
	 * Cipher.getInstance("DES/CBC/PKCS5Padding"); dcipher =
	 * Cipher.getInstance("DES/CBC/PKCS5Padding");
	 * 
	 * // CBC requires an initialization vector ecipher.init(Cipher.ENCRYPT_MODE,
	 * key, paramSpec); dcipher.init(Cipher.DECRYPT_MODE, key, paramSpec); } catch
	 * (java.security.InvalidAlgorithmParameterException e) { System.err.println(e);
	...
	private static void printCryptoImpls(String serviceType) {
		...
		for (int i = 0; i < providers.length; i++) {
			...
			/*
			 * for (Iterator it=keys.iterator(); it.hasNext(); ) { String key =
