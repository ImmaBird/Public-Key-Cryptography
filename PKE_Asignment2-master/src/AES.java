...
public class AES {
	...
	private final byte[] iv = new byte[] { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b,
			0x0c, 0x0d, 0x0e, 0x0f };
	...
	private static String[] getServiceTypes() {
		...
		for (int i = 0; i < providers.length; i++) {
			...
			for (Iterator it = keys.iterator(); it.hasNext();) {
				...
				if (debug)
					System.out.println("\t" + key.substring(0, ix));
			}
		}
		...
		for (int i = 0; i < providers.length; i++) {
			...
			for (Iterator it = keys.iterator(); it.hasNext();) {
				...
				if (key.startsWith(serviceType + ".")) {
					...
					if (debug)
						System.out.println("\t" + key.substring(serviceType.length() + 1));
				} else if (key.startsWith("Alg.Alias." + serviceType + ".")) {
					// This is an alias
					result.add(key.substring(serviceType.length() + 11));
					if (debug)
						System.out.println("\t" + key.substring(serviceType.length() + 11));
				} else {
					if (debug)
	...
	/**
	 * Encrypts incoming stream and places it in another stream (good for file
	 * encryption).
	 * 
	...
	/**
	 * Decrypts incoming stream and places it in another stream (good for file
	 * decryption).
	 * 
